...
4. 我们提供了一个测试链接进行以上测试 [https://m-stg2.tianxiaxinyong.com/cooperation/b-test.html](https://m-stg2.tianxiaxinyong.com/cooperation/b-test.html)
...
* 微信支付完成回调：由天下信用后台为用户生成"`URL Scheme`"标识，或开发者自行生成"`URL Scheme`"标识 （下面以"`PYTXXY`"来示范）, 在项目工程里面的`info`选项卡下`URL Type`分组添加一项，`URL Schemes`的值为"`PYTXXY`"。添加以后在手机上运行一次项目，然后在手机`Safari`浏览器里面输入刚添加的"`PYTXXY://`"，验证是否能跳转到自己的应用
